...
class VAE(nn.Module):
    ...
    def reconstruct(self, data, angle_idx):
        self.eval()
        with torch.no_grad():
            qz_x = self.qz_x(*self.enc(data, angle_idx))
            px_z_params = self.dec(qz_x.rsample(), angle_idx)

        return get_mean_param(px_z_params)

    def forward(self, x, angle_idx, K=1, no_dec=False):
        qz_x = self.qz_x(*self.enc(x, angle_idx))
        zs = qz_x.rsample(torch.Size([K]))
        if no_dec:
            return qz_x, zs

        t = self.dec(zs, angle_idx)
