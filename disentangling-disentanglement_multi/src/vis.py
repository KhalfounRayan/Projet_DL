...
def visualize_latent_space(model, data_loader, method='PCA', num_samples=1000, save_path=None):
    ...
    for batch_idx, (data, label, angle_idx) in enumerate(data_loader):
        print(f"[DEBUG] Batch {batch_idx}: Data shape: {data.shape}")

        data = data.view(data.size(0), -1).to(next(model.parameters()).device)
        angle_idx = angle_idx.to(data.device)  # angle_idx doit être sur le même device que data
        mu, _ = model.encode(data, angle_idx)   # passer angle_idx à encode()
